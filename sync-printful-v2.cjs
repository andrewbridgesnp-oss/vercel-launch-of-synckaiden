...
// Use PNG files only (converted from WEBP)
const designs = [
...
// Printful product IDs - using simpler products that work better with API
const productTypes = [
  { type: "T-Shirt", printfulId: 71, variantIds: [4012, 4013, 4014, 4016, 4017, 4018, 4019] }, // Bella+Canvas 3001 (S-2XL)
  { type: "Hoodie", printfulId: 146, variantIds: [4320, 4321, 4322, 4324, 4325, 4326, 4327] }, // Gildan 18500 (S-2XL)
];
...
async function createPrintfulProduct(design, productType, fileId) {
  try {
    ...
    const syncVariants = productType.variantIds.map(variantId => ({
      retail_price: productType.type === 'T-Shirt' ? '24.99' : '44.99',
    ...
    console.log(`✅ Created: ${design.name} - ${productType.type} (${syncVariants.length} sizes)`);
    return response.data.result.id;
  } catch (error) {
...
async function syncAllProducts() {
  console.log('🚀 Starting Printful sync with URL-based uploads...\n');
