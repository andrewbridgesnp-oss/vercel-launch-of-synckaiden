...
// Gildan 18500 Hoodie - Black only, S-2XL (CORRECT variant IDs)
const hoodieVariants = [5530, 5531, 5532, 5533, 5534]; // S, M, L, XL, 2XL
...
async function createPrintfulHoodie(design, fileId) {
  try {
    // Create sync variants for all sizes
    const syncVariants = hoodieVariants.map(variantId => ({
      retail_price: '44.99',
    ...
    console.log(`✅ Created: ${design.name} - Hoodie (${syncVariants.length} sizes)`);
    return response.data.result.id;
  } catch (error) {
    console.error(`❌ Error creating ${design.name} - Hoodie:`, error.response?.data?.result || error.message);
    return null;
  }
}

async function syncHoodies() {
  console.log('🧥 Starting Printful sync for HOODIES...\n');
  console.log(`📦 Total designs: ${designs.length}`);
  console.log(`👕 Sizes per hoodie: ${hoodieVariants.length} (S-2XL)`);
  console.log(`📊 Total hoodies to create: ${designs.length}\n`);
  ...
  for (const design of designs) {
    ...
    const result = await createPrintfulHoodie(design, fileId);
    if (result) {
      successCount++;
    } else {
      failCount++;
    ...
    // Wait 3 seconds before next design
    await new Promise(resolve => setTimeout(resolve, 3000));
  }
  
  console.log('\n\n✨ Hoodie sync complete!');
  console.log(`✅ Successfully created: ${successCount} hoodies`);
  console.log(`❌ Failed: ${failCount} hoodies`);
  console.log(`📊 Success rate: ${Math.round((successCount / (successCount + failCount)) * 100)}%`);
  console.log(`\n🧥 Total hoodie variants: ${successCount * hoodieVariants.length} (${hoodieVariants.length} sizes each)`);
}

syncHoodies().catch(console.error);
