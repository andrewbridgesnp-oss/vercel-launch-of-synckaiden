...
import { getDecryptedJSON, setEncryptedJSON } from "@/lib/secureStorage";
import { hasTierAccess, TIER_ORDER } from "@/lib/tiers";
...
const WORKFLOW_LIBRARY = [
  {
    id: "idea-llc",
    title: "Idea → LLC safety check + launch",
    tier: "Builder Sync",
    education: "Covers the intake questions founders skip: ownership split, filing state, registered agent, and bank readiness.",
    execution: [
      "Validate name and domain availability; record conflicts",
      "Capture ownership and decision rights before filing",
      "Generate state-specific filing checklist with docs and fees",
      "Prep EIN + bank compliance packet (no submissions without approval)",
    ],
    automation: "Auto-generate filing packet drafts, but require human approval for every state submission and bank outreach.",
    risk: "Not legal advice. Escalate to a business attorney before signing or submitting anything.",
  },
  {
    id: "ap-guardrails",
    title: "Accounts payable guardrails",
    tier: "Operator Sync",
    education: "Teaches segregation of duties, approval ladders, and vendor verification before payments leave the account.",
    execution: [
      "Import vendors and flag missing W-9/contract data",
      "Set approval rules by amount and vendor risk tier",
      "Queue payments only after dual review; log evidence",
      "Send digest with anomalies and blocked attempts",
    ],
    automation: "Can draft payment queues and reminders but must wait for dual sign-off and audit logging before release.",
    risk: "No automatic payments without CFO/controller approval. Suspected fraud routes to human escalation immediately.",
  },
  {
    id: "agent-hardening",
    title: "AI agent rollout hardening",
    tier: "Empire Sync",
    education: "Covers sandboxes, kill switches, and observability so agents cannot run wild or leak data.",
    execution: [
      "Define scopes, forbidden actions, and rate limits",
      "Set staged environments with synthetic data first",
      "Add approval hooks for spending, account changes, and identity actions",
      "Log inputs/outputs with redaction; ship weekly evals",
    ],
    automation: "Allow schedule-based jobs with runtime caps and a manual kill switch that pauses every task immediately.",
    risk: "Escalate to security/legal if an agent touches PII/PHI or financial rails. Never bypass audit logging.",
  },
  {
    id: "client-onboarding",
    title: "Client onboarding + risk triage",
    tier: "Starter Sync",
    education: "Walks a new services client through goals, data boundaries, and acceptable automation levels.",
    execution: [
      "Capture objectives, timelines, and forbidden outcomes",
      "Assign data owners and backup contacts",
      "Publish a runbook for approvals, rollbacks, and communications",
      "Schedule a 30-day review with metrics and risks",
    ],
    automation: "Auto-generate the onboarding packet and calendar invites; no system access is provisioned automatically.",
    risk: "If the client requests scope that touches regulated data, require written approval and professional counsel.",
  },
];

async function loadState() {
  try {
    const parsed = await getDecryptedJSON(LS_KEY);
...
async function saveState(state) {
  await setEncryptedJSON(LS_KEY, state);
}
...
export default function Academy() {
  ...
  const [dailyMinutes, setDailyMinutes] = useState(DEFAULT_STATE.profile.timeBudgetMin || 120);
  ...
  const [onboardingForm, setOnboardingForm] = useState({
    ...
    northStar: DEFAULT_STATE.profile.northStar,
    timeBudgetMin: DEFAULT_STATE.profile.timeBudgetMin,
    tier: DEFAULT_STATE.profile.tier,
    riskAcknowledged: false,
  });
  ...
  useEffect(() => {
    ...
    loadState().then((loaded) => {
      if (!active) return;
      setState(loaded);
      setDailyMinutes(loaded.profile?.timeBudgetMin || DEFAULT_STATE.profile.timeBudgetMin);
      setOnboardingForm({
        name: loaded.profile?.name || DEFAULT_STATE.profile.name,
        northStar: loaded.profile?.northStar || DEFAULT_STATE.profile.northStar,
        timeBudgetMin: loaded.profile?.timeBudgetMin || DEFAULT_STATE.profile.timeBudgetMin,
        tier: loaded.profile?.tier || DEFAULT_STATE.profile.tier,
        riskAcknowledged: Boolean(loaded.profile?.riskAcknowledged),
      });
      setOnboardingOpen(!loaded.onboardingComplete);
      setHydrated(true);
    });
    return () => {
      active = false;
  ...
  useEffect(() => {
    ...
    isProUnlocked().then((val) => {
      if (active) setProUnlockedState(Boolean(val));
    });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    setOnboardingForm((form) => ({
      ...form,
      name: state.profile?.name || form.name,
      northStar: state.profile?.northStar || form.northStar,
      timeBudgetMin: state.profile?.timeBudgetMin || form.timeBudgetMin,
      tier: state.profile?.tier || form.tier,
      riskAcknowledged: Boolean(state.profile?.riskAcknowledged),
    }));
  }, [hydrated, onboardingOpen, state.profile]);

  const streakTodayDone = useMemo(() => isSameDay(state.streak?.lastDoneISO || ""), [state]);
  const currentTier = state.profile?.tier || TIER_ORDER[0];
  const workflowAudit = state.workflowAudit || [];
  const onboardingNeeded = hydrated && !state.onboardingComplete;
  ...
  function logWorkflowExecution(workflow, mode) {
    const entry = {
      id: uid(),
    ...
    setState((s) => ({
      ...s,
      workflowAudit: [entry, ...(s.workflowAudit || [])].slice(0, 30),
    }));
    setWorkflowNotice(`${workflow.title} logged with ${mode}. Keep evidence and approvals attached.`);
  }

  function handleWorkflowStart(workflow, mode) {
    if (!hasTierAccess(currentTier, workflow.tier)) {
      setWorkflowNotice(`Upgrade to ${workflow.tier} to run “${workflow.title}” with proper guardrails.`);
      window.dispatchEvent(new Event("synckaiden:openPricing"));
      return;
    }
    logWorkflowExecution(workflow, mode);
  }

  function completeOnboarding() {
    if (!onboardingForm.riskAcknowledged) {
      setWorkflowNotice("Acknowledge the risk notice before proceeding.");
      return;
    }

    const sanitizedName = onboardingForm.name.trim() || state.profile.name || DEFAULT_STATE.profile.name;
    const minutes = Number(onboardingForm.timeBudgetMin) || DEFAULT_STATE.profile.timeBudgetMin;
    ...
    setState((s) => ({
      ...
      profile: {
        ...s.profile,
        name: sanitizedName,
        northStar: onboardingForm.northStar || s.profile.northStar,
        timeBudgetMin: minutes,
        tier: onboardingForm.tier,
    ...
    <div className="min-h-screen bg-background text-foreground">
      {showGate && !proUnlocked ? (
        <ProGate
          ...
          onUnlocked={() => {
            isProUnlocked().then((val) => setProUnlockedState(Boolean(val)));
            setShowGate(false);
          }}
        />
      ) : null}

      <Modal open={onboardingOpen} onOpenChange={setOnboardingOpen} title="Security-first onboarding">
        <div className="space-y-3 text-sm">
          <div className="grid gap-2">
            <div className="text-xs font-semibold">Name</div>
            <Input value={onboardingForm.name} onChange={(e) => setOnboardingForm((f) => ({ ...f, name: e.target.value }))} />
          </div>

          <div className="grid gap-2">
            <div className="text-xs font-semibold">North Star</div>
            <Textarea
              value={onboardingForm.northStar}
              onChange={(e) => setOnboardingForm((f) => ({ ...f, northStar: e.target.value }))}
            />
          </div>

          <div className="grid gap-2">
            <div className="text-xs font-semibold">Time budget (minutes per session)</div>
            <Input
              type="number"
              min={30}
              value={onboardingForm.timeBudgetMin}
              onChange={(e) => setOnboardingForm((f) => ({ ...f, timeBudgetMin: Number(e.target.value || 0) }))}
            />
          </div>
          ...
          <div className="grid gap-2">
            ...
            <select
              ...
              onChange={(e) => setOnboardingForm((f) => ({ ...f, tier: e.target.value }))}
            >
              {TIER_ORDER.map((tier) => (
            ...
            <div className="text-xs text-muted-foreground">Higher tiers unlock automation drafts and financial workflows.</div>
          </div>

          <label className="flex items-start gap-3 rounded-xl bg-muted/60 p-3">
            <Checkbox
              checked={onboardingForm.riskAcknowledged}
              onCheckedChange={(v) => setOnboardingForm((f) => ({ ...f, riskAcknowledged: Boolean(v) }))}
            />
            <div className="text-xs text-muted-foreground">
              I understand this academy is educational only. Automation steps require human approval, audit logging, and
              professional escalation for legal/financial decisions.
            </div>
          </label>

          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" onClick={() => setOnboardingOpen(false)}>
              Close
            </Button>
            <Button onClick={completeOnboarding}>Save onboarding</Button>
          </div>
        </div>
      </Modal>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                {onboardingNeeded ? <Badge variant="destructive">Onboarding pending</Badge> : <Badge variant="secondary">Risk ack</Badge>}
              </div>
          ...
          <TabsContent value="ship" className="mt-4">
            ...
            <Card className="mb-4 rounded-2xl">
              <CardHeader>
                <SectionTitle
                  icon={Shield}
                  title="Workflow library with tiered guardrails"
                  subtitle="Every run logs what happened and reminds you when to escalate to a professional."
                />
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-2">
                  {WORKFLOW_LIBRARY.map((wf) => {
                    const allowed = hasTierAccess(currentTier, wf.tier);
                    return (
                      <div key={wf.id} className="rounded-2xl border border-border bg-background p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="font-semibold">{wf.title}</div>
                            <div className="text-xs text-muted-foreground">{wf.education}</div>
                          </div>
                          <Badge variant={allowed ? "secondary" : "outline"}>{wf.tier}</Badge>
                        </div>
                        <div className="mt-3 grid gap-1 text-sm">
                          {wf.execution.map((step, idx) => (
                            <div key={idx}>• {step}</div>
                          ))}
                        </div>
                        <div className="mt-3 rounded-xl bg-muted p-2 text-xs text-muted-foreground">
                          Automation: {wf.automation}
                        </div>
                        <div className="mt-2 rounded-xl bg-amber-50 p-2 text-xs text-amber-900">
                          Risk: {wf.risk} Escalate to a licensed professional when unsure.
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Button
                            size="sm"
                            disabled={!allowed}
                            onClick={() => handleWorkflowStart(wf, "manual")}
                            variant={allowed ? "default" : "secondary"}
                          >
                            {allowed ? "Log execution" : `Requires ${wf.tier}`}
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleWorkflowStart(wf, "automation-draft")}
                          >
                            Automation draft
                          </Button>
                        </div>
                ...
                <div className="rounded-2xl bg-muted p-3">
                  <div className="font-semibold">Audit log (last 30)</div>
                  {workflowAudit.length === 0 ? (
                    <div className="mt-2 text-sm text-muted-foreground">Run a workflow to populate the audit trail.</div>
                  ) : (
                    <div className="mt-2 grid gap-2">
                      {workflowAudit.map((entry) => (
                        <div key={entry.id} className="rounded-xl bg-background p-3 text-sm">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="font-semibold">{entry.name}</div>
                            <Badge variant="secondary">{entry.tier}</Badge>
                          </div>
                          <div className="text-xs text-muted-foreground">{new Date(entry.timeISO).toLocaleString()} • {entry.mode}</div>
                          <div className="text-xs text-muted-foreground">{entry.risk}</div>
                        </div>
                      ))}
                    </div>
                  )}
